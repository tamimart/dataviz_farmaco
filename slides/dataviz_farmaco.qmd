...
-   [Slides <](https://tamimart.quarto.pub/dataviz_farmaco/)
...
ggplot(mort_plot, aes(x = tempo_obito_horas, y = sobreviventes, color = grupo)) +
  ...
  labs(title = NULL,
       x = "Tempo (horas)",
       y = "Sobreviventes",
       caption = "Fonte: Dados simulados a partir de Marinho et al. (não publicado)") +
  theme_classic(base_size = 20) +
...
data <- tibble(
  x = 1:5,
  y = 1:5,
  grupo = c("Principal", "Principal", "Destaque", "Principal", "Principal")
)

ggplot(data, aes(x = x, y = y, fill = grupo)) +
  geom_point(shape = 21, size = 6, color = "white") +
  scale_fill_manual(
    values = c("Principal" = "grey70", "Destaque" = color2)
  ) +
  labs(
